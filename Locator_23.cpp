...
void setup() {
    ...
    Particle.function("stAccThrHld", setAccelThreshhold);
    Particle.function("stFenceCoord", setGeoFenceCoord);
    Particle.function("stSpdThrHld", setSpeedThreshhold);
    Particle.function("stGeoThrHld", setGeoThreshhold); 
    
    Particle.function("stDelMinFst", setDelayMinutesFast);
    Particle.function("stDelMinMed", setDelayMinutesMedium);
    Particle.function("stDelMinSlow", setDelayMinutesSlow);
...
void loop() {
    ...
    // If the actual speed is at or above the threshhold speed then it is assumed that the asset is moving
    ...
    // +Accel
    // -Geo
    if ((assetSpeed >= speedThreshhold) && (assetAccellXYZ >= accellThreshhold) && (geoFenceDistance >= geoThreshhold)){ 
    ...
    // +Accel
    // -Geo
    else if ((assetSpeed < speedThreshhold) && (assetAccellXYZ >= accellThreshhold) && (geoFenceDistance >= geoThreshhold)){ 
