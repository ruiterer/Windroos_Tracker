...
int transmittingData = 1; // Specific for ThingSpeak function
int publishOn = 1;  // Specific for publish function
...
void setup() {
    ...
    Particle.function("publishMode", publishMode);
...
void loop() {
    ...
    if (GPS.fix) {
        ...
        // While loop to read the Lat and Lon and verify whether one of the readings is 0. This occurs once in a while and is not correct
        // To prevent the system to stay in the loop, a counter is used.
        while(((rawLatitude=0) || (rawLongitude=0)) && (gpsCounter<5)) {
            
            rawLatitude = convertDegMinToDecDeg(GPS.latitude);
            rawLongitude = convertDegMinToDecDeg(GPS.longitude);
            gpsCounter++;
            
        }
        
        // Convert the raw latitude and longitude to string in order to use these in further processing
        sParams[0]=String(rawLatitude);
        sParams[1]=String(rawLongitude);
    ...
    // -Geo
        ...
        // A collision outside of the geo fence while not moving should also be published
        if (publishOn == 1 ) {
            // Publish an event when asset leaves the geofence
            Particle.publish("Windroos","registreert botsing buiten geofence");
        }    
    ...
    // +Geo
        ...
        // A collision in the geo fence while not moving should also be published
        if (publishOn == 1 ) {
            // Publish an event when asset leaves the geofence
            Particle.publish("Windroos","registreert botsing in geofence");
        }    
    ...
    if (inGeoFenceStatus == 0  && inGeoFenceStatus != previousInGeoFenceStatus && publishOn == 1 ) {
        // Publish an event when asset leaves the geofence
    ...
    if (millis()-lastPublish > delayMinutes*60*1000) {
        ...
        // if (GPS.fix) {   // Optional if statement to only transmit data when there is a gps fix
            ...
            if (transmittingData) {
                ...
                // Turn on the blue led to indicate that communication has taken place
...
// Allows you to remotely change whether a device is publishing to ThingSpeak
...
// or is only reporting data over Serial and ThingSpeak. Saves data when using only Serial!
// Change the default at the top of the code.
int publishMode(String command) {
    publishOn = atoi(command);
    return 1;
};
